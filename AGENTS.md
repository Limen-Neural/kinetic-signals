...
Standard commands are documented in the **Build & test** and **Running the demo** sections above. The notes below cover only the non-obvious environment caveats.
...
- **`--all-features` / `sentry` needs system OpenSSL:** The `sentry` feature pulls in `openssl-sys`, which requires the `libssl-dev` and `pkg-config` system packages (already present in the snapshot). Without them, `cargo build --all-features` or `cargo test --all-features` will fail with a "Could not find directory of OpenSSL installation" error. The default (no-feature) build and test runs have zero dependencies and do not require these packages.
- **`Cargo.lock` is gitignored** (library crate), so it is regenerated on a fresh checkout; run `cargo fetch` to pre-warm the dependency cache.
