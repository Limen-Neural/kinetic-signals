...
- **`--all-features` / `sentry` needs system OpenSSL:** The `sentry` feature pulls in `openssl-sys`, which requires the `libssl-dev` and `pkg-config` system packages (already present in the snapshot). The default (no-feature) build and test runs have zero dependencies and do not require these packages.
- **Diagnosing the failure:** Missing the above packages causes `cargo build --all-features` or `cargo test --all-features` to fail with a "Could not find directory of OpenSSL installation" error.
